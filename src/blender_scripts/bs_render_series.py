...
def render_reference(imaging_type, wl, dry_run=True):
    ...
    if imaging_type == imaging_type_refl:
        render_target(
            imaging_type, wl, target_refl_ref, render_path_refl_ref, dry_run=dry_run
        )
    elif imaging_type == imaging_type_tran:
        render_target(
            imaging_type, wl, target_tran_ref, render_path_tran_ref, dry_run=dry_run
        )
...
def clear_folders(clear_reference=False):
    ...
    list(
        map(
            os.unlink,
            (os.path.join(render_path_leaf, f) for f in os.listdir(render_path_leaf)),
        )
    )
    if clear_reference:
        ...
        list(
            map(
                os.unlink,
                (
                    os.path.join(render_path_refl_ref, f)
                    for f in os.listdir(render_path_refl_ref)
                ),
            )
        )
        list(
            map(
                os.unlink,
                (
                    os.path.join(render_path_tran_ref, f)
                    for f in os.listdir(render_path_tran_ref)
                ),
            )
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        key_base_path[0],
        key_base_path[1],
        dest=key_base_path[1],
        action="store",
        required=True,
        help="Base path to render folders. Under it have to be folders for "
    ...
    parser.add_argument(
        ...
        action="store_true",
        required=False,
        help="If given, nothing is saved but the prints will come out.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        required=False,
        help="Clear leaf render folder before starting new renders.",
    )

    parser.add_argument(
        key_clear_refs[0],
        key_clear_refs[1],
        dest=key_clear_refs[1],
        action="store_true",
        required=False,
        help="Clear all reference folders before starting new renders.",
    )

    parser.add_argument(
        key_render_references[0],
        key_render_references[1],
        dest=key_render_references[1],
        action="store_true",
        required=False,
        help="Render new references if illumination changes.",
    )

    parser.add_argument(
        key_wavelength[0],
        key_wavelength[1],
        dest=key_wavelength[1],
        action="store",
        required=True,
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="List of Shader volume absorption node's density input.",
    )

    parser.add_argument(
        key_scat_dens[0],
        key_scat_dens[1],
        dest=key_scat_dens[1],
        action="store",
        required=True,
        type=str,
    ...
    wavelength_list = vars(args)[key_wavelength[1]]
    abs_dens_list = vars(args)[key_abs_dens[1]]
    scat_dens_list = vars(args)[key_scat_dens[1]]
    scat_ai_list = vars(args)[key_scat_ai[1]]
    mix_fac_list = vars(args)[key_mix_fac[1]]
    wavelength_list = [
        float(f) for f in (wavelength_list.lstrip("[").rstrip("]")).split(", ")
    ]
    abs_dens_list = [
        float(f) for f in (abs_dens_list.lstrip("[").rstrip("]")).split(", ")
    ]
    scat_dens_list = [
        float(f) for f in (scat_dens_list.lstrip("[").rstrip("]")).split(", ")
    ]
    scat_ai_list = [
        float(f) for f in (scat_ai_list.lstrip("[").rstrip("]")).split(", ")
    ]
    mix_fac_list = [
        float(f) for f in (mix_fac_list.lstrip("[").rstrip("]")).split(", ")
    ]

    if (
        len(wavelength_list) != len(abs_dens_list)
        or len(wavelength_list) != len(scat_dens_list)
        or len(wavelength_list) != len(scat_ai_list)
        or len(wavelength_list) != len(mix_fac_list)
    ):
        raise ValueError(
            f"One or more of the parameter lists length do not match the number of wavelengths."
        )
