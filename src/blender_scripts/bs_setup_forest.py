...
def set_leaf_material(
    leaf_material_name, band_list, ad_list, sd_list, ai_list, mf_list
):
    ...
    for i, band in enumerate(band_list):
        FU.set_material_parameter_per_frame(
            leaf_material_name, "Absorption density", ad_list[i], band
        )
        FU.set_material_parameter_per_frame(
            leaf_material_name, "Scattering density", sd_list[i], band
        )
        FU.set_material_parameter_per_frame(
            leaf_material_name, "Scattering anisotropy", ai_list[i], band
        )
        FU.set_material_parameter_per_frame(
            leaf_material_name, "Mix factor", mf_list[i], band
        )
        FU.set_material_parameter_per_frame(
            leaf_material_name, "Density scale", DENSITY, band
        )
        ...
        for row in reader:
            if leaf_material_name in row[0]:
                rgba = (
                    ...
                    float(row[3]),
                    1.0,
                )  # must have four items so adding 1 for alpha
                bpy.data.materials[leaf_material_name].node_tree.nodes["Group"].inputs[
                    "RGB color"
                ].default_value = rgba
...
def read_leaf_material_csv(file_name: str):
    ...
    if not os.path.exists(p):
        raise FileNotFoundError(
            f"Leaf csv file '{p}' not found. Check your file names given to setup script."
        )
...
def set_animation_frames(band_count):
    ...
    for scene in b_data.scenes:
        ...
        scene.frame_start = 1
        scene.frame_end = (
            band_count  # can be safely set to zero as Blender will fix it into one
        )
...
def insert_leaf_data(leaf_material_names: str):
    ...
    for i, leaf_csv_name in enumerate(lmn):
        ...
        try:
            band_list, wl_list, ad_list, sd_list, ai_list, mf_list = (
                read_leaf_material_csv(leaf_csv_name)
            )
        except FileNotFoundError as e:
            logging.fatal(
                f"Could not find file '{leaf_csv_name}'. Blend file will not be setup properly."
            )
            raise
        ...
        set_leaf_material(
            leaf_material_name=leaf_csv_name,
            band_list=band_list,
            ad_list=ad_list,
            sd_list=sd_list,
            ai_list=ai_list,
...
def insert_soil_data():
    ...
    for i, band in enumerate(bands):
        ...
        material.node_tree.nodes["Group"].inputs[socket_name].default_value = (
            reflectances[i]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=False,
        type=str,
        help="List of available leaf names as a string.",
    )
