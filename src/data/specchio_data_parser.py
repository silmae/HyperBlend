...
def make_target(wls, r_m, t_m, path, sample_idx):
    ...
    if len(wls) != len(r_m) or len(wls) != len(t_m):
        raise ValueError(
            f"Length of the lists of wavelenghts ({len(wls)}), reflectances "
            f"({len(r_m)}) or transmittances ({len(t_m)}) did not match."
        )

    target_data = np.zeros((3, len(wls)))
    wls = np.array(wls)
    r_m = np.array(r_m).clip(0.0, 1.0)
    t_m = np.array(t_m).clip(0.0, 1.0)
    target_data[0] = wls
    target_data[1] = r_m
    target_data[2] = t_m
...
def collect_pairs():
    ...
    for i, sample_dict in enumerate(sample_dict_list):
        ...
        for j in range(i + 1, len(sample_dict_list)):
            ...
            if (
                sample_id == sample_id_other
                and is_adaxial == is_adaxial_other
                and is_shaded == is_shaded_other
                and is_reflectance != is_reflectance_other
            ):
