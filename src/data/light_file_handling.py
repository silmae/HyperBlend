...
Files are formatted so that comment lines are prefixed with '#' and rest of the lines
contain wavelength-irradiance pairs that can be cast to floats and be read
directly. For spectra generated with either `NASA PSG <https://psg.gsfc.nasa.gov/>`_
or , `SSolar GOA <https://doi.org/10.5194/gmd-15-1689-2022>`_  the files are
...
def read_light_file(path: str):
    ...
    The file is expected to be one generated by either NASA PSG or SSolar GOA. If it
    is, it will be reformatted to align with how HyperBlend expects the light files
    to look like. Otherwise, it will be read as is, provided that the file is formatted
    so that comment lines are prefixed with '#' and the rest of the lines contain
    wavelength-irradiance pairs that can be cast to floats.
    """
    ...
    if not is_psg and not is_goa:
        logging.warning(
            ...
            f"prefixed with '#' and rest of the lines contain wavelength-irradiance pairs "
